...
var (
	...
	timedCmd     = runCmd.Command("timed", "Schedule requests based on timestamps.")
	floodCmd     = runCmd.Command("flood", "Ignore timestamps (send requests as fast as possible).")
	tickerCmd    = runCmd.Command("ticker", "Schedule requests with a certain frequency.")
	tickerFreq   = tickerCmd.Flag("freq", "Ticker frequency [1/s].").Default("1").Float()
	prefix       = kingpin.Flag("prefix", "String to prepend to urls.").PlaceHolder("http://example.com").Default("").String()
